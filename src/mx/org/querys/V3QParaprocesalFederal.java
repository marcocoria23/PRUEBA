 ...
 */
public class V3QParaprocesalFederal {
...
//Query de validacion donde el  año de la fecha de apertura sea diferente al año de la clave del expediente dependiendo del año judicial del estado de Campeche.
    public ArrayList Año_JudicialCampeche() {
        ...
        sql = "SELECT * FROM (\n"
                ...
                + "        from TR_FED_paraprocesal ) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO) WHERE SUBSTR(CLAVE_ORGANO,0,2)='" + PValidacion.clave_entidad + "'  \n"
...
//Query de validacion donde el año de la fecha de apertura es diferente al año de la clave del expediente.
    public ArrayList Año_Expe_ParaprocesalNE() {
        ...
        sql = "SELECT * FROM(\n"
                ...
                + "select  clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy') FECHA_APERTURA_EXPEDIENTES ,SUBSTR(TO_CHAR(FECHA_APERTURA_EXPEDIENTE, 'DD-MM-YYYY'),-4,4) \n"
                + "FECHA_APERTURA_EXPEDIENTE,SUBSTR(regexp_replace(expediente_clave, '[^0-9]', ''),-4,4)  EXPE_AÑO\n"
                + "from TR_FED_paraprocesal ) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO AND EXPE_AÑO NOT IN ('2021','2022','2023','2020','2024','2025')) WHERE SUBSTR(CLAVE_ORGANO,0,2)='" + PValidacion.clave_entidad + "' \n"
                + "or clave_organo='" + PValidacion.clave_organo + "'";
        System.out.println(sql);
        resul = conexion.consultar(sql);
        try {
    ...
    public ArrayList FECHA_APERTURA_EXPEDIENTE_FUT() {
        ...
        sql = "SELECT clave_organo, expediente_clave, TO_CHAR(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY')FECHA_APERTURA_EXPEDIENTE,\n"
                + "TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM  TR_FED_PARAPROCESAL  \n"
                + "WHERE \n"
    ...
    public ArrayList FECHA_PRESENTA_SOLI_FUT() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT clave_organo, expediente_clave, TO_CHAR(FECHA_PRESENTA_SOLI,'DD/MM/YYYY')FECHA_PRESENTA_SOLI,\n"
                + "TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM  TR_FED_PARAPROCESAL  \n"
                + "WHERE \n"
    ...
    public ArrayList FECHA_ADMISION_SOLI_FUT() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT clave_organo, expediente_clave, TO_CHAR(FECHA_ADMISION_SOLI,'DD/MM/YYYY')FECHA_ADMISION_SOLI,\n"
                + "TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM  TR_FED_PARAPROCESAL  \n"
                + "WHERE \n"
    ...
    public ArrayList FECHA_CONCLUSION_EXPE_FUT() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT clave_organo, expediente_clave, TO_CHAR(FECHA_CONCLUSION_EXPE,'DD/MM/YYYY')FECHA_CONCLUSION_EXPE,\n"
                + "TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM  TR_FED_PARAPROCESAL  \n"
                + "WHERE \n"
    ...
    public ArrayList Duplicidad_expediente() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,TO_CHAR(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY')FECHA_APERTURA_EXPEDIENTE,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2 FROM TR_FED_PARAPROCESAL\n"
                ...
                + "select CLAVE_ORGANO,expediente_clave,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2 FROM TR_FED_PARAPROCESAL \n"
    ...
    public ArrayList Fecha_PresentacionNE() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "select * from (select ESTATUS_EXPEDIENTE,CLAVE_ORGANO,EXPEDIENTE_CLAVE,to_char(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')FECHA_APERTURA_EXPEDIENTE,to_char(FECHA_PRESENTA_SOLI,'dd/mm/yyyy')FECHA_PRESENTA_SOLI\n"
                + "from TR_FED_PARAPROCESAL\n"
                + "WHERE to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy') < to_date(FECHA_PRESENTA_SOLI,'dd/mm/yyyy')) where  \n"
                + "FECHA_PRESENTA_SOLI <> '09/09/1899'  and clave_organo='" + PValidacion.clave_organo + "' or substr(clave_organo,0,2)='" + PValidacion.clave_entidad + "' \n"
                + "and FECHA_PRESENTA_SOLI <> '09/09/1899' \n";
        System.out.println(sql);
        resul = conexion.consultar(sql);
        try {
    ...
    public ArrayList Fecha_PresentacionAdmiNE() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "select * from (select ESTATUS_EXPEDIENTE,CLAVE_ORGANO,EXPEDIENTE_CLAVE,to_char(FECHA_ADMISION_SOLI,'dd/mm/yyyy')FECHA_ADMISION_SOLI,to_char(FECHA_PRESENTA_SOLI,'dd/mm/yyyy')FECHA_PRESENTA_SOLI \n"
                + "from TR_FED_PARAPROCESAL \n"
                + "WHERE to_date(FECHA_ADMISION_SOLI,'dd/mm/yyyy') < to_date(FECHA_PRESENTA_SOLI,'dd/mm/yyyy')) where  \n"
                + "FECHA_PRESENTA_SOLI <> '09/09/1899'  and clave_organo='" + PValidacion.clave_organo + "' or substr(clave_organo,0,2)='" + PValidacion.clave_entidad + "' \n"
                + "and FECHA_PRESENTA_SOLI <> '09/09/1899' \n";
        System.out.println(sql);
        resul = conexion.consultar(sql);
    ...
    public ArrayList Fecha_Admision_SoliNE() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "select * from (select ESTATUS_EXPEDIENTE,clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY') FECHA_APERTURA_EXPEDIENTE,to_char(FECHA_ADMISION_SOLI,'DD/MM/YYYY') FECHA_ADMISION_SOLI \n"
                + "from TR_FED_paraprocesal where \n"
                + " FECHA_ADMISION_SOLI <> '09/09/1899'  and clave_organo='" + PValidacion.clave_organo + "' or substr(clave_organo,0,2)='" + PValidacion.clave_entidad + "' \n"
                + "and FECHA_ADMISION_SOLI <> '09/09/1899')\n"
                + "where  to_date(FECHA_ADMISION_SOLI,'dd/mm/yyyy')  < to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')";
        System.out.println(sql);
        resul = conexion.consultar(sql);
        try {
            while (resul.next()) {
...
//Query de validacion donde la fecha de conclusion del expediente no debe de ser menor a la fecha de apertura del expediente.
    public ArrayList Fecha_Conclusion_ExpeNE() {
        conexion.Conectar();
        Array = new ArrayList();
        sql = "select * from (select ESTATUS_EXPEDIENTE,clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY') FECHA_APERTURA_EXPEDIENTE,to_char(Fecha_Conclusion_Expe,'DD/MM/YYYY') Fecha_Conclusion_Expe , comentarios\n"
                + "from TR_FED_paraprocesal where \n"
                + " Fecha_Conclusion_Expe <> '09/09/1899'  and clave_organo='" + PValidacion.clave_organo + "' or substr(clave_organo,0,2)='" + PValidacion.clave_entidad + "' \n"
                + "and Fecha_Conclusion_Expe <> '09/09/1899')\n"
                + "where  to_date(Fecha_Conclusion_Expe,'dd/mm/yyyy')  < to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')";
        System.out.println(sql);
        resul = conexion.consultar(sql);
        try {
            while (resul.next()) {
