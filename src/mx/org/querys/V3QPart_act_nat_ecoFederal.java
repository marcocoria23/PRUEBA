 ...
 */
public class V3QPart_act_nat_ecoFederal {
...
"from(\n" +
"select * from(\n" +
"select * from TR_FED_COLECT_ECONOM where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
") where expediente_clave not in (select expediente_clave from TR_FED_part_act_COLECT_ECONOM where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE)PRIN LEFT JOIN TR_FED_COLECT_ECONOM SEC\n" +
"ON PRIN.CLAVE_ORGANO=SEC.CLAVE_ORGANO AND PRIN.EXPEDIENTE_CLAVE=SEC.EXPEDIENTE_CLAVE  \n" +
