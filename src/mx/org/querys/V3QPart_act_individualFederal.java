...
public class V3QPart_act_individualFederal {
...
"from(\n" +
"select * from(\n" +
"select * from TR_FED_INDIVIDUAL where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
") where expediente_clave not in (select expediente_clave from TR_FED_part_ACT_INDIVIDUAL where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE)PRIN LEFT JOIN TR_FED_INDIVIDUAL SEC\n" +
"ON PRIN.CLAVE_ORGANO=SEC.CLAVE_ORGANO AND PRIN.EXPEDIENTE_CLAVE=SEC.EXPEDIENTE_CLAVE  \n" +
