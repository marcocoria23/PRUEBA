 ...
 */
public class V3QColec_EconomFederal {
...
"        from tr_fed_colect_econom) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
...
public ArrayList Año_Expe_ColecEconomNE(){
      ...
      sql="SELECT * FROM(\n" +
        ...
        "from tr_fed_colect_econom) WHERE  FECHA_APERTURA_EXPEDIENTE <>EXPE_AÑO AND EXPE_AÑO NOT IN ('2021','2022','2023','2020','2024','2025')) WHERE SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
        "or clave_organo='"+PValidacion.clave_organo+"'";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
"TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM tr_fed_COLECT_ECONOM \n" +
"WHERE \n" +
...
public ArrayList FECHA_ADMISION_DEMANDA_FUT(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT clave_organo, expediente_clave,TO_CHAR(FECHA_ADMISION_DEMANDA,'DD/MM/YYYY')FECHA_ADMISION_DEMANDA,\n" +
"TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM tr_fed_COLECT_ECONOM \n" +
"WHERE \n" +
...
public ArrayList FECHA_AUDIENCIA_ECONOM_FUT(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT clave_organo, expediente_clave,TO_CHAR(FECHA_AUDIENCIA_ECONOM,'DD/MM/YYYY')FECHA_AUDIENCIA_ECONOM,\n" +
"TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM tr_fed_COLECT_ECONOM \n" +
"WHERE \n" +
...
public ArrayList FECHA_RESOLUCION_FUT(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT clave_organo, expediente_clave,  TO_CHAR(FECHA_RESOLUCION,'DD/MM/YYYY')FECHA_RESOLUCION,\n" +
"TO_CHAR(SYSDATE,'DD/MM/YYYY') FROM tr_fed_COLECT_ECONOM \n" +
"WHERE \n" +
...
public ArrayList Duplicidad_expediente(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,TO_CHAR(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY')FECHA_APERTURA_EXPEDIENTE,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2 FROM tr_fed_colect_econom\n" +
...
"select CLAVE_ORGANO,expediente_clave,TO_NUMBER(regexp_replace(expediente_clave, '[^0-9]', ''))expediente_clave2 from tr_fed_colect_econom \n" +
...
"WHERE to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy') < to_date(FECHA_PRES_DEMANDA,'dd/mm/yyyy')) where  \n" +
   ...
   public ArrayList Fecha_PresentacionAdmiNE(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (select entidad_clave,CLAVE_ORGANO,EXPEDIENTE_CLAVE,to_char(FECHA_ADMISION_DEMANDA,'dd/mm/yyyy')FECHA_ADMISION_DEMANDA,to_char(fecha_pres_demanda,'dd/mm/yyyy')fecha_pres_demanda \n" +   
"from tr_fed_COLECT_ECONOM \n" +
"WHERE to_date(FECHA_ADMISION_DEMANDA,'dd/mm/yyyy') < to_date(FECHA_PRES_DEMANDA,'dd/mm/yyyy')) where \n" +
          " fecha_pres_demanda <> '09/09/1899'  and clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
          "and fecha_pres_demanda <> '09/09/1899'\n";
      System.out.println(sql);
      resul=conexion.consultar(sql);
 ...
 //Query de validacion donde la fecha de admision no debe de ser menor a la fecha de apertura del expediente
    public ArrayList Fecha_Admision_DemandaNE(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (select entidad_clave,clave_organo,expediente_clave,FECHA_APERTURA_EXPEDIENTE,to_char(Fecha_Admision_Demanda,'DD/MM/YYYY') Fecha_Admision_Demanda \n" +
          "from tr_fed_colect_econom where  \n" +
          " Fecha_Admision_Demanda <> '09/09/1899'  and clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
          "and Fecha_Admision_Demanda <> '09/09/1899'\n" +
          ") where  to_date(Fecha_Admision_Demanda,'dd/mm/yyyy')  < to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
          while (resul.next()) {
    ...
    public ArrayList Fecha_audiencia_EconomNE(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (select entidad_clave,clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY') FECHA_APERTURA_EXPEDIENTE,to_char(Fecha_audiencia_Econom,'DD/MM/YYYY') Fecha_audiencia_Econom \n" +
          "from tr_fed_colect_econom where \n" +
          " Fecha_audiencia_Econom <> '09/09/1899'  and clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
          "and Fecha_audiencia_Econom <> '09/09/1899'\n" +
          " ) where to_date(Fecha_audiencia_Econom,'dd/mm/yyyy')  <= to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
          while (resul.next()) {
    ...
    public ArrayList Fecha_resolucionNE(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (select entidad_clave,clave_organo,expediente_clave,to_char(FECHA_APERTURA_EXPEDIENTE,'DD/MM/YYYY') FECHA_APERTURA_EXPEDIENTE,to_char(Fecha_resolucion,'DD/MM/YYYY') Fecha_resolucion \n" +
          "from tr_fed_colect_econom where " +
          " Fecha_resolucion <> '09/09/1899'  and clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
          "and Fecha_resolucion <> '09/09/1899'\n" +
          ") where to_date(Fecha_resolucion,'dd/mm/yyyy')  < to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy')";
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
          while (resul.next()) {
...
"FROM tr_fed_AUDIENCIAS P,tr_fed_colect_econom S\n" +
...
"AND P.TIPO_PROCED=5 AND to_date(FECHA_AUDIEN_CELEBRADA,'dd/mm/yyyy') < to_date(FECHA_APERTURA_EXPEDIENTE,'dd/mm/yyyy') \n" +
  ...
  public ArrayList Fecha_Aud_Admision(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select * from (SELECT S.ENTIDAD_CLAVE,P.CLAVE_ORGANO,P.EXPEDIENTE_CLAVE,to_char(P.FECHA_AUDIEN_CELEBRADA,'DD/MM/YYYY')FECHA_AUDIEN_CELEBRADA,to_char(S.FECHA_ADMISION_DEMANDA,'DD/MM/YYYY') FECHA_ADMISION_DEMANDA, P.TIPO_PROCED,P.ID_AUDIENCIA\n" +
"FROM tr_fed_AUDIENCIAS P,tr_fed_colect_econom S\n" +
"WHERE P.CLAVE_ORGANO=S.CLAVE_ORGANO AND P.EXPEDIENTE_CLAVE=S.EXPEDIENTE_CLAVE AND \n" +
"P.TIPO_PROCED=5 AND to_date(FECHA_AUDIEN_CELEBRADA,'dd/mm/yyyy') < to_date(FECHA_ADMISION_DEMANDA,'dd/mm/yyyy') \n" +
               " )where \n" +
          " FECHA_AUDIEN_CELEBRADA <> '09/09/1899'  and FECHA_ADMISION_DEMANDA <> '09/09/1899' and  clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"FROM\n" +
"    tr_fed_COLECT_ECONOM\n" +
"WHERE\n" +
"     modif_condiciones = 2\n" +
"     AND nuevas_condiciones = 2\n" +
"     AND suspension_temporal = 2\n" +
"     AND terminacion_colectiva = 2\n" +
"     AND otro_motivo_econom = 2 and(SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
" or clave_organo='"+PValidacion.clave_organo+"')"; 
System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
