 ...
 */
public class V3QPart_act_colectivoFederal {
...
"from(\n" +
"select * from(\n" +
"select * from TR_FED_colectivo where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
" and incompetencia=2 and estatus_demanda=1 OR CLAVE_ORGANO='"+PValidacion.clave_organo+"' \n" +
" and incompetencia=2 and estatus_demanda=1) where CANTIDAD_ACTORES>0\n" +
") where expediente_clave not in (select expediente_clave from TR_FED_part_ACT_colectivo where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"' \n" +
...
"from TR_FED_part_ACT_colectivo\n" +
"where substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"\n" +
"and id_actor not like '%-%' \n" +
"group by clave_organo,expediente_clave) prin  right join TR_FED_part_ACT_colectivo sec\n" +
"on prin.clave_organo=sec.clave_organo and prin.expediente_clave=sec.expediente_clave\n" +
"where substr(sec.clave_organo,0,2)='"+PValidacion.clave_entidad+"' \n" +
"and sec.id_demandado not like '%-%' \n" +
"group by sec.clave_organo,sec.expediente_clave,prin.desglose_actores) prin  right join  TR_FED_COLECTIVO sec\n" +
"on  prin.clave_organo=sec.clave_organo and  prin.expediente_clave=sec.expediente_clave\n" +
"where substr(sec.clave_organo,0,2)='"+PValidacion.clave_entidad+"' ) where clave_organo='"+PValidacion.clave_organo+"' or substr(clave_organo,0,2)='"+PValidacion.clave_entidad+"')\n" +
...
"\n" +
"GROUP BY CLAVE_ORGANO,EXPEDIENTE_CLAVE)PRIN LEFT JOIN TR_FED_COLECTIVO SEC\n" +
"ON PRIN.CLAVE_ORGANO=SEC.CLAVE_ORGANO AND PRIN.EXPEDIENTE_CLAVE=SEC.EXPEDIENTE_CLAVE  \n" +
