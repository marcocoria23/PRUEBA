 ...
 */
public class V3Part_Act_individualFederal {
...
public ArrayList ActorNI(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(Actor,'99','No identificado') Actor FROM TR_FED_PART_ACT_INDIVIDUAL\n" +
...
public ArrayList Actor_otro(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(ACTOR,'6','Beneficiario','7','Otro') ACTOR FROM TR_FED_PART_ACT_INDIVIDUAL\n" +
"WHERE (ACTOR IN (6,7)  AND (SEXO IS NOT NULL OR  EDAD  IS NOT NULL OR OCUPACION  IS NOT NULL OR\n" +
"NSS  IS NOT NULL OR CURP  IS NOT NULL OR RFC_TRABAJADOR  IS NOT NULL OR JORNADA  IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
