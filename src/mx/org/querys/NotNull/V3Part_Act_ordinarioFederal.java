 ...
 */
public class V3Part_Act_ordinarioFederal {
...
public ArrayList ActorNI(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(Actor,'99','No identificado') Actor FROM TR_FED_PART_ACT_ORDINARIO\n" +
...
public ArrayList Actor_Trabajador(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(Actor,'1','Trabajador') Actor FROM TR_FED_PART_ACT_ORDINARIO\n" +
...
"IS NOT NULL OR  MUJERES IS NOT NULL OR NO_IDENTIFICADO IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' )) "; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Actor_Sindicato(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(Actor,'3','sindicato') Actor  FROM TR_FED_PART_ACT_ORDINARIO\n" +
"WHERE (ACTOR IN (3)  AND (SEXO IS NOT NULL OR  EDAD IS NOT NULL OR OCUPACION IS NOT NULL OR NSS IS NOT NULL OR\n" +
"CURP IS NOT NULL OR RFC_TRABAJADOR IS NOT NULL OR JORNADA IS NOT NULL)AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Actor_Coalicion(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(Actor,'4','Coalicion_de_trabajadores') Actor FROM TR_FED_PART_ACT_ORDINARIO\n" +
