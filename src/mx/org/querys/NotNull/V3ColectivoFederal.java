 ...
 */
public class V3ColectivoFederal {
...
public ArrayList Tipo_Asunto(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(TIPO_ASUNTO,'9','No_identificado') TIPO_ASUNTO from TR_FED_COLECTIVO\n" +
...
public ArrayList Cons_Expedida(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(CONSTANCIA_CONS_EXPEDIDA,'2','No','9','No identificado') CONSTANCIA_CONS_EXPEDIDA FROM TR_FED_COLECTIVO\n" +
"WHERE (CONSTANCIA_CONS_EXPEDIDA IN (2,9) AND INCOMPETENCIA=1 AND (CONSTANCIA_CLAVE IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Suspencion_temporal(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(SUSPENSION_TMP,'2','No','9','No identificado') SUSPENSION_TMP FROM TR_FED_COLECTIVO\n" +
"WHERE (SUSPENSION_TMP IN (2,9) AND TIPO_ASUNTO=2 AND (NO_IMPUTABLE_ST IS NOT NULL OR INCAPACIDAD_FISICA_ST IS NOT NULL OR FALTA_MATERIA_PRIM_ST IS NOT NULL OR  FALTA_MINISTRACION_ST IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Terminacion_Colectiva(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(TERMINACION_TRAB,'2','No','9','No identificado') TERMINACION_TRAB FROM TR_FED_COLECTIVO\n" +
"WHERE (TERMINACION_TRAB in (2,9) and TIPO_ASUNTO=2  AND (FUERZA_MAYOR_TC IS NOT NULL OR INCAPACIDAD_FISICA_TC IS NOT NULL OR QUIEBRA_LEGAL_TC IS NOT NULL OR AGOTAMIENTO_MATERIA_TC IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Viola_Derechos(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(VIOLA_DERECHOS,'2','No','9','No identificado') VIOLA_DERECHOS FROM TR_FED_COLECTIVO\n" +
"WHERE (VIOLA_DERECHOS=2 AND TIPO_ASUNTO=2 AND (LIBERTAD_ASOCIACION IS NOT NULL OR  LIBERTAD_SINDICAL IS NOT NULL \n" +
"OR  DERECHO_COLECTIVA IS NOT NULL OR OTRO_COLECTIVA IS NOT NULL OR OTRO_ESP_COLECTIVA IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Estatus_Demanda(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(ESTATUS_DEMANDA,'9','No_identificado') ESTATUS_DEMANDA,REPLACE(COMENTARIOS,',','')COMENTARIOS from TR_FED_COLECTIVO\n" +
...
public ArrayList Estatus_ExpedienteNI(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="select CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(ESTATUS_EXPEDIENTE,'9','No_identificado') ESTATUS_EXPEDIENTE,REPLACE(COMENTARIOS,',','')COMENTARIOS from TR_FED_COLECTIVO\n" +
...
public ArrayList IncompetenciaNI(){
      ...
      Array = new ArrayList();
      sql="select * from(\n" +
"select CLAVE_ORGANO,EXPEDIENTE_CLAVE,INCOMPETENCIA,REPLACE(COMENTARIOS,',','')COMENTARIOS from TR_FED_COLECTIVO\n" +
...
public ArrayList Tipo_IncompetenciaNI(){
      ...
      sql="select * from(\n" +
" SELECT * FROM(\n" +
"SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(INCOMPETENCIA,'2','No') INCOMPETENCIA,TIPO_INCOMPETENCIA FROM TR_FED_COLECTIVO WHERE INCOMPETENCIA=2)\n" +
...
public ArrayList PivIncompetencia(){
      ...
      Array = new ArrayList();
      sql="SELECT * FROM(\n" +
"SELECT CLAVE_ORGANO, EXPEDIENTE_CLAVE, ENTIDAD_CLAVE, MUNICIPIO_CLAVE,\n" +
...
"ESTATUS_EXPEDIENTE,FECHA_ACTO_PROCESAL,FASE_SOLI_EXPEDIENTE,FORMA_SOLUCION_AD,OTRO_ESP_SOLUCION_AD,FECHA_DICTO_DEPURACION_AD,\n" +
...
"FECHA_DICTO_DEPURACION_AD IS NOT NULL OR\n" +
...
"MONTO_SOLUCION_AJ IS NOT NULL )) where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' "; 
      System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Preve_demanda(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(PREVE_DEMANDA,'2','No','9','No identificado') PREVE_DEMANDA FROM TR_FED_COLECTIVO\n" +
...
public ArrayList Estatus_Demanda_Desechada(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT * FROM (SELECT CLAVE_ORGANO, EXPEDIENTE_CLAVE, ENTIDAD_CLAVE, MUNICIPIO_CLAVE,\n" +
...
public ArrayList Tram_depuracion(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(AUTO_DEPURACION,'2','No','9','No identificado') AUTO_DEPURACION FROM TR_FED_COLECTIVO \n" +
"WHERE (AUTO_DEPURACION in (2,9) and ESTATUS_DEMANDA=1 AND (FECHA_DEPURACION IS NOT NULL) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' ))"; 
      //System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Audiencia_juicio(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(AUDIENCIA_JUICIO,'2','No','9','No identificado') AUDIENCIA_JUICIO FROM TR_FED_COLECTIVO \n" +
...
public ArrayList Estatus_Expediente(){
      ...
      Array = new ArrayList();
      sql="SELECT * FROM (\n" +
               "SELECT * FROM (SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,ESTATUS_EXPEDIENTE FROM TR_FED_COLECTIVO WHERE \n" +
"FASE_SOLI_EXPEDIENTE IS NOT NULL OR FORMA_SOLUCION_AD IS NOT NULL OR OTRO_ESP_SOLUCION_AD IS NOT NULL OR\n" +
"FECHA_DICTO_DEPURACION_AD IS NOT NULL OR TIPO_SENTENCIA_AD IS NOT NULL OR MONTO_SOLUCION_AD IS NOT NULL OR\n" +
"FORMA_SOLUCION_AJ IS NOT NULL OR OTRO_ESP_SOLUCION_AJ IS NOT NULL OR FECHA_RESOLUCION_AJ IS NOT NULL OR\n" +
"TIPO_SENTENCIA_AJ IS NOT NULL OR MONTO_SOLUCION_AJ IS NOT NULL) where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
...
"from TR_FED_COLECTIVO WHERE ESTATUS_EXPEDIENTE=1 AND\n" +
...
"FROM TR_FED_COLECTIVO P, TC_FED_FASE_EXPEDIENTE S\n" +
"WHERE  P.FASE_SOLI_EXPEDIENTE=S.ID\n" +
"AND FASE_SOLI_EXPEDIENTE NOT IN(2,3,99) AND (SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"'  )"; 
System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
...
public ArrayList Fase_Sol_expNI(){
      conexion.Conectar();
      Array = new ArrayList();
      sql="SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,DECODE(FASE_SOLI_EXPEDIENTE,'99','No identificado')FASE_SOLI_EXPEDIENTE FROM TR_FED_COLECTIVO \n" +
...
public ArrayList Fase_Sol_exp_TAP(){
      ...
      sql="SELECT * FROM(\n" +
"SELECT * FROM(\n" +
"SELECT CLAVE_ORGANO,EXPEDIENTE_CLAVE,FASE_SOLI_EXPEDIENTE FROM TR_FED_COLECTIVO WHERE \n" +
"FORMA_SOLUCION_AJ IS NOT NULL OR OTRO_ESP_SOLUCION_AJ IS NOT NULL OR FECHA_RESOLUCION_AJ IS NOT NULL OR \n" +
"TIPO_SENTENCIA_AJ IS NOT NULL OR MONTO_SOLUCION_AJ IS NOT NULL) where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
...
"FECHA_DICTO_DEPURACION_AD IS NOT NULL OR  MONTO_SOLUCION_AD IS NOT NULL \n" +
" ) where  SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
...
"     AND otro_conflicto = 2 and(SUBSTR(CLAVE_ORGANO,0,2)='"+PValidacion.clave_entidad+"'  \n" +
" or clave_organo='"+PValidacion.clave_organo+"' )"; 
System.out.println(sql);
      resul=conexion.consultar(sql);
      try {
